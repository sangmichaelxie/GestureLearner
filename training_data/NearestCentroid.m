function NearestCentroid()
	...
	for m = min_endpoint:max_endpoint
	    ...
	    for i = 1:iterations    
	        ...
	        C = findCentroids(X_train, y_train, numClasses);
	...
	title('Nearest Centroid Bias and Variance');
...
function C = findCentroids(X_train, y_train, numCentroids)
    ...
    sumClasses = zeros(1, numCentroids);
    for i = 1:size(X_train, 1)
        C(y_train(i) + 1, :) = C(y_train(i) + 1, :) + X_train(i, :);   
        sumClasses(y_train(i) + 1) = sumClasses(y_train(i) + 1) + 1;
    end
